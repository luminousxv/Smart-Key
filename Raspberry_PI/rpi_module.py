...
try:
    ...
    while True:
        ...
        if res_code == 200:                     # 서버에 정상적으로 연결됨
            print("정상적으로 연결되었습니다.")
            if res_message == 'open':           # key state가 open인 경우
                doAngle(open_angle)             # servo모터 open
                display.lcd_clear()
                display.lcd_display_string("Key_state: open", 1)   # display 출력
                sleep(3)
            if res_message == 'close':          # key state가 close인 경우
                doAngle(close_angle)
                display.lcd_clear()
